...
Right-click and open the image below in its own window or tab. 
...
We changed our working directory to .ssh
We copied the key there
We changed the permissions on the key so that only we can read and write it (ssh requires this).
